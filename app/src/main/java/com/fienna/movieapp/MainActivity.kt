...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isLanguageChecked){
            val appLocale: LocaleListCompat = LocaleListCompat.forLanguageTags(SettingFragment.languageIn)
            AppCompatDelegate.setApplicationLocales(appLocale)
        }else{
            val appLocale: LocaleListCompat = LocaleListCompat.forLanguageTags(SettingFragment.languageEn)
            AppCompatDelegate.setApplicationLocales(appLocale)
        }
    }
}